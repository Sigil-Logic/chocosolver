...
The reference itself was produced twice in succession and the two manifests were byte-identical.  Publishing a capture as the frozen directory is `python3 scripts/publish-choco4-baseline.py baseline-out .evidence/choco4-baseline` (validates the capture, then writes a deterministic tarball), followed, after any hand edit such as this README, by `python3 scripts/publish-choco4-baseline.py --seal .evidence/choco4-baseline`, which writes `evidence.sha256`.  `bash scripts/test-choco-baseline-tools.sh` runs 32 synthetic positive and negative cases against the three tools (permuted siblings, truncated and mis-terminated dumps, altered sets, changed exits, missing models, renamed exclusions, missing or emptied artifacts, hash-only sides, order drift, empty or tampered captures, nesting, sealing).
