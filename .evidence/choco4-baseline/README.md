...
| clafer compiler (the three `.cfr` models) | Sigil clafer fork, commit `cefa7aa35f3b32c1811b2283fe715ed32081e7ae`, `Clafer 0.5.1`.  Master advanced to `0e3f958` after the capture (HTML/Graph printer spelling; a positioned decline of `let`, which no input uses); recompiling every `.cfr` input of both baseline halves with a build of `0e3f958` reproduced the frozen `model.js` files byte-for-byte, so either commit regenerates this manifest |
